...
pm = model.pivot_polynomials.coefficients;
...
frmt_pm = [ repmat([mf ', '], 1, size(pm,1)-1) [mf '; \n']];
...
fprintf(prob.fid, [tab '// Pivot polynomials:\n']);
for ii = 1 : size(pm, 2)
  fprintf(prob.fid, ...
          [tab prob.pn '.pm.col(' num2str(ii-1) ') << ' frmt_pm], ...
          pm(:, ii));
end
