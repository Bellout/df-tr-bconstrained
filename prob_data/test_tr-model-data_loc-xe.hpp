...
namespace TestResources {
    ...
    public:
        ...
        TrustRegionModelData() {
            ...
            prob1.obp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.obp.col(0) <<   1.1054181264452054,  -2.1575682495753270,  -0.0958878575287296,   1.8931687706300306,   0.0483868557174898,   0.0451613139951083; 
            prob1.v_obp.push_back(prob1.obp);
            // Trial point found (output data):
            //   1.1159521053218844,   0.9275767160463136; 
            ...
            prob1.obp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.obp.col(0) <<   1.7334598164392465,  -2.1789879314504512,  -0.5907144738414207,   1.7886727676599785,   0.0405556909997611,   0.2413364395936488; 
            prob1.v_obp.push_back(prob1.obp);
            // Trial point found (output data):
            //   1.1671643538151035,   2.2515425071373132; 
            ...
            prob1.pnp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.pnp.col(0) <<   0.0000000000000000,   1.2904485507392136,  -1.2986718799505361,  -1.7513081132367800,   0.3360925608088236,   2.0000000000000000; 
            prob1.v_pnp.push_back(prob1.pnp);

            // New points (output data) - > new_points = [new_point_max, new_point_min];:
            //  -0.0363263390296650,  -4.0288536833167123; 
            //   5.2723822980866224,  -0.2366685022622553; 
            ...
            prob1.obp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.obp.col(0) <<   0.9999999999999998,  -2.0000000000000004,   0.0000000000000006,   2.0000000000000009,   0.0000000000000002,  -0.0000000000000001; 
            prob1.v_obp.push_back(prob1.obp);
            // Trial point found (output data):
            //   1.0000000013288068,   0.9275767160463105; 
            ...
            prob1.obp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.obp.col(0) <<  -0.1219498875984083,   0.1159521066506910,   0.0064660751934397,   0.0000000000000000,   0.0000000000000000,   0.0000000000000000; 
            prob1.v_obp.push_back(prob1.obp);
            // Trial point found (output data):
            //  -0.1613225707905918,  -0.2337458560730824; 
            ...
            prob1.pnp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.pnp.col(0) <<   0.0000000000000000,  -0.1159521039930774,  -0.0064660751934397,   2.0000000000000000,   0.0000000000000000,   0.0000000000000000; 
            prob1.v_pnp.push_back(prob1.pnp);

            // New points (output data) - > new_points = [new_point_max, new_point_min];:
            //  -1.1613217519755161,  -1.1610133066684460; 
            ...
            prob1.obp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.obp.col(0) <<   0.9534333300469710,  -0.7009820909115257,  -1.0759392534745149,   0.4336617596228564,   0.3860925295373951,   0.3965688619335375; 
            prob1.v_obp.push_back(prob1.obp);
            // Trial point found (output data):
            //   0.4193390382013571,   1.5082379389429059; 
            ...
            prob1.pnp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.pnp.col(0) <<   0.0000000000000000,   0.1199620329839517,  -0.1604746633912337,  -2.0691652648426704,   1.0000000000000000,   0.0000000000000000; 
            prob1.v_pnp.push_back(prob1.pnp);

            // New points (output data) - > new_points = [new_point_max, new_point_min];:
            //  -0.5806612755450501,   0.5806612590741010; 
            //  -0.2226490061062863,  -0.5806602420113399; 
            ...
            prob1.obp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.obp.col(0) <<   1.0046720476856419,  -1.6207565024593928,  -0.2499336438992923,   1.8470566095417014,  -0.2344097789001399,   0.3296654305553035; 
            prob1.v_obp.push_back(prob1.obp);
            // Trial point found (output data):
            //   1.0688685286269908,   1.5080614230067848; 
            ...
            prob1.pnp.resize(6,1);
            // Polynomial [-polynomial_max] (input data):
            prob1.pnp.col(0) <<   0.0000000000000000,   0.0537942600945861,  -1.0832403345517432,  -0.9278703575359735,  -1.4221071254288851,   2.0000000000000000; 
            prob1.v_pnp.push_back(prob1.pnp);

            // New points (output data) - > new_points = [new_point_max, new_point_min];:
            //   0.5806610513032245,  -0.5806612600038307; 
            //   0.5806609808899866,   0.5806607510723465; 
