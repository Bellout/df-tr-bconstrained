...
while true
  ...
  if ~isempty(pivot_index) && ~complete_only
    ...
    if pivot_found
      ...
      fprintf(prob.fid_evaluatePolynomial, ['[ --> improveModel[a] ]               ']);
    ...
    if ~interpolation_set_changed && ~complete_only
      ...
      if ~isempty(pivot_index)
        ...
        if pivot_found
            ...
            fprintf(prob.fid_evaluatePolynomial, ['[ --> improveModel[b] ]               ']);
            [new_value prob] = evaluate_polynomial(chosen_poly, new_point, prob);
            ...
            [indices_pts_quad2, pivot_absvalues_quad2, ...
             ...
             polynomials_permutation2] = ...
                check_geometry_quadratic_mn(...
                                            points_scaled(:, linear_basis_size+1:end), ...
                                            quadratic_pivot_polynomials, tol_pivot);
            %%
            1;
        else
...
if linear_interpolation
  ...
  for m = 1:points_used_num
    for n = 1:linear_basis_size
      ...
      fprintf(prob.fid_evaluatePolynomial, ['[ --> improveModel[c] ]               ']);
...
%         uf = linsolve(L, P*fvalues', l_opts);
  ...
  for m = 1:points_used_num
    for n = 1:basis_size
      fprintf(prob.fid_evaluatePolynomial, ['[ --> improveModel[d] ]               ']);
      [M(m, n) prob] = evaluate_polynomial(basis(n), points_scaled(:, m), prob);
    end
  end
