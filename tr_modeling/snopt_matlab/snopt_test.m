...
xfrmt = repmat('%6.3f ', 1, size(x0,1));
ffrmt = repmat('%6.3f ', 1, size(f0,1));
gfrmt = repmat('%6.3f ', 1, size(g0,1));
...
% ----------------------------------------------------------
xub = repmat(10, size(x0));
xlb = repmat(-10, size(x0));
...
SNOPT(D.x, D.xlow, D.xupp, D.Flow, D.Fupp, 'SNOPT_f',...
